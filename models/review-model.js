 ...
 * ************************** */
async function getReviewsBymodel_id(inv_id) {
  try {
    const sql =
      "SELECT review_text, review_date, account_firstname, account_lastname FROM review JOIN account ON review.account_id = account.account_id WHERE review.inv_id = $1 ORDER BY review_date DESC";
...
async function getReviewsByAccountId(account_id) {
  try {
    const sql =
      "SELECT review_id, review_date, inv_make, inv_model, inv_year FROM review JOIN inventory ON review.inv_id = inventory.inv_id WHERE review.account_id = $1 ORDER BY review_date DESC";
...
async function getReviewById(review_id) {
  try {
    const sql =
      "SELECT review_id, review_text, review_date, inv_make, inv_model, inv_year FROM review JOIN inventory ON review.inv_id = inventory.inv_id WHERE review_id = $1";
